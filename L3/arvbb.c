...
int igual(Arv *r, Arv *s){
	...
	if (r == NULL && s != NULL) //Se o nó r estiver vazio o s não
        return 1; //Retorna para não serem iguais
    if (r != NULL && s == NULL) //Se o nó r não estiver vazio o s sim
        return 1; //Retorna para não serem iguais
	if (r->info == s->info){ //Verifica se os infos dos nós de ambas as árvores são iguais
